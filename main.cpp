...
public:
    ...
    void addItem(const string& itemName, int quantity, double price) {
        ...
        cout << "         Current total: " << totalAmount << endl;
    }
    ...
    public:
        void loadBooks(const string& filePath, Inventory &inventory) {
            ...
            while (getline(file, line)) {
                ...
                string name, author, genre, series, language;
                int pubYear, stock, booksSold;
                float price, rating;
                bool newCollection;
                ss >> quoted(name) >> quoted(author) >> quoted(genre) >> pubYear >> price >> stock >> newCollection >> quoted(series) >> booksSold >> rating >> quoted(language);
        ...
        void loadMagazines(const string& filePath, Inventory &inventory) {
            ...
            while (getline(file, line)) {
                stringstream ss(line);
                string title, publisher, genre, language;
                int pubYear, stock, issuesSold;
                float price;
                bool newCollection;
                ss >> quoted(title) >> quoted(publisher) >> quoted(genre) >> pubYear >> price >> stock >> newCollection >> quoted(language) >> issuesSold;
